...
async function loadReceipt(txHash: string): Promise<{ receipt: TransactionReceipt; vaults: Map<string, BasketVault> }> {
  ...
  let list = await listBasketVaults();
  const vaults = new Map(list.map((v) => [v.address.toLowerCase(), v]));
  // A vault created after the cache was filled: refresh once before giving up.
  if (!receipt.logs.some((l) => vaults.has(l.address.toLowerCase()))) {
    list = await listBasketVaults(true);
    for (const v of list) vaults.set(v.address.toLowerCase(), v);
  }
