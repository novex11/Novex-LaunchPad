...
SHARE_IMPL="$(cast call "$PAIR_DEPLOYER" 'shareTokenImplementation()(address)' --rpc-url "$RPC")"
...
if [[ "$NETWORK" == "testnet" ]]; then
  ...
  verify_one "$SHARE_IMPL" src/PairShareToken.sol:PairShareToken --constructor-args=0x
...
ℹ️   Mainnet explorer verification must be submitted from a browser (Standard JSON input,
    ...
    The three implementations make every launched pair and token verified instantly:
      PairVault       $VAULT_IMPL
      PairShareToken  $SHARE_IMPL
